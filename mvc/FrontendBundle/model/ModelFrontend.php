...
class ModelFrontend {
	...
	public function selectCursosByUsuario($id_usuario)
	{
		$database = new Database();
		$query = 'SELECT c.pk_curso as pk_curso, c.nombre as cursoName FROM cursos_usuarios cu JOIN
		          cursos c ON c.pk_curso = cu.fk_curso
		          WHERE cu.fk_usuario = '.$id_usuario.' AND c.status = 1 
		          ORDER BY c.pk_curso desc';
		//echo $query; die();
	...
	public function selectTemasActivosByPkCurso($pk_curso)
	{
		$database = new Database();
		$query = '	SELECT * FROM temas WHERE fk_curso = '.$pk_curso.' AND status = 1 
	...
	public function selectFicherosActivosByPkTemaType($pk_tema, $type)
	{
		$database = new Database();
		$query = '	SELECT * FROM ficheros WHERE fk_tema = '.$pk_tema.'
					AND tipo = '.$type.' AND status = 1 
	...
	{
		$database = new Database();
		$query = '	SELECT u.pk_usuario as pk_usuario, u.nombre as nombreUsuario, 
